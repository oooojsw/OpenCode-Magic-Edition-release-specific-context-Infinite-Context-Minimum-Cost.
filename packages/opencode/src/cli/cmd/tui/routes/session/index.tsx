...
import type { ReleaseContextTool } from "@/tool/release-context"
  ...
  return (
    <Show when={!shouldHide()}>
      <Switch>
        ...
        <Match when={props.part.tool === "release_context"}>
          <ReleaseContext {...toolprops} />
        </Match>
        <Match when={true}>
...
function ReleaseContext(props: ToolProps<typeof ReleaseContextTool>) {
  const { theme } = useTheme()
  const count = createMemo(() => props.metadata.releasedCount ?? 0)
  const saved = createMemo(() => props.metadata.savedTokens ?? 0)

  return (
    <Switch>
      <Match when={props.output}>
        <BlockTool title="# Released Context" part={props.part}>
          <box gap={1}>
      ...
      <Match when={true}>
        <InlineTool icon="🗑" pending="Releasing context..." complete={count()} part={props.part}>
          Released {count()} tool call{count() !== 1 ? "s" : ""}, saved ~{saved()} tokens
        </InlineTool>
      </Match>
    </Switch>
  )
